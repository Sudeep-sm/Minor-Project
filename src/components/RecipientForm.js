...
export default function RecipientForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await API.post("/api/recipients", form);

      alert("Recipient registered successfully!");


      const recipientData = res.data.recipient;
      const certificateText = `Certificate of Appreciation\n\nThis is to certify that ${recipientData.name} has registered as a recipient with OrganHub.\nWe wish you the best on your health journey.`;

      // Removed automatic certificate download to allow manual download from certificate view
      // const blob = new Blob([certificateText], { type: "application/pdf" });
      // saveAs(blob, `recipient_certificate_${recipientData.name}.pdf`);

      navigate("/recipient-dashboard");
    } catch (err) {
      console.error(" Recipient Form Error:", err.response?.data || err.message);
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-center mb-4 text-primary">Recipient Registration</h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-2">
            <select name="organNeeded" className="form-control" onChange={handleChange} required>
